    ...
    <div class="mx-auto">
      <div v-if="this.currentCareerRank?.CurrentProgress && this.careerRankData?.Ranks" class="d-flex p-3 flex-column text-white main">
        <div>
          ...
          <img :url="`https://gamecms-hacs.svc.halowaypoint.com/hi/Images/file/${careerRankData.Ranks[currentCareerRank.CurrentProgress.Rank].RankIcon}`">
        </div>
        <hr class="m-0">
    ...
    export default {
      ...
      methods: {
        async getCurrentCareerRank() {
          await axios.get(`http://127.0.0.1:8000/servicerecord/careerrank/${this.$route.params.gamertag}`)
                .then(response => {
